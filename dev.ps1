[CmdletBinding(PositionalBinding=$false)] param([Parameter(ValueFromRemainingArguments=$true)][string[]]$Arguments)
...
& wsl.exe env "OPENSAGETV_VIBE_BUILD_ENV_ROOT=$linuxBuildEnv" bash "$linuxRoot/dev.sh" @Arguments; exit $LASTEXITCODE
